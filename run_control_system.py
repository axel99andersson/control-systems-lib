...
attacker = ct.NonlinearIOSystem(
    None, attacker_output, name="attacker",
    inputs=["v"], outputs=["v_attacked"], params={"attack_strength": 0.1}
)
...
interconnected_attacked_system = ct.InterconnectedSystem(
    ...
    connections=(
        ...
        ("attacker.v", "vehicle.v"),     # Attacker receives delayed velocity
        # ("delay.v", "vehicle.v"),             # Delay input is the vehicle velocity

    ),
    inplist=["control.vref", "vehicle.gear", "vehicle.theta"],
    outlist=["control.u", "vehicle.v", "attacker.v_attacked"],
    outputs=['u', 'v', 'va']
)
...
X0, U0, Y0 = ct.find_eqpt(
    interconnected_attacked_system, [vref[0], 0], [vref[0], gear[0], theta0[0]],
...
theta_hill = [
    ...
    4./180. * pi for t in T]
breakpoint()
t, y = ct.input_output_response(interconnected_attacked_system, T, [vref, gear, theta_hill], X0)
